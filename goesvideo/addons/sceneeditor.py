...
class GoesSceneEditor():
    def __init__(self, base_dir, scene_folder=None, session_name=None, reset_session=False):
        """
        ...
        if not session_name:
            self.output_path = Path(base_dir) / "SceneEdits" / scene_folder / "Default_Session"
            self.session_name = "Default_Session"
        else:
            self.output_path = Path(base_dir) / "SceneEdits" / scene_folder / session_name
    ...
    def resize(self, newwidth, newheight, resample=None, preview=False, force=False):
        ...
        if not force and not preview:
            if self.geodata['image']['raster_profile']['height'] == newheight:
                if self.geodata['image']['raster_profile']['width'] == newwidth:
                    print(f"{Fore.RED}Images in Scene: {self.scene_folder}, Session: {self.session_name} are already "
                          f"sized at the requested dimensions. Skipping resizing.")
                    return
        
        print(f"{Fore.GREEN}Resizing images in Scene: {self.scene_folder}, Session: {self.session_name}...")
        time.sleep(0.5)
        ...
        for f in tqdm(self.img_filenames, colour='green'):
            ...
            if self.type_is_geotiff: 
                _, gdict = gistools.resize(infile, newwidth, newheight, resample=resample, outfile=outfile)
            else:
                _, gdict = gistools.resize(infile, newwidth, newheight, geodata=geodata, resample=resample, outfile=outfile)
        ...
        if self.type_is_geotiff:
            gistools.update_geodata(str(self.output_path / self.img_filenames[0]), geodata)
    ...
    def reproject(self, dst_crs, preview=False, force=False):
        ...
        print(f"{Fore.GREEN}Reprojecting images in Scene: {self.scene_folder}, Session: {self.session_name}...")
        time.sleep(0.5)
        ...
        for f in tqdm(self.img_filenames, colour='green'):
            ...
            if self.type_is_geotiff:
              ...
              _, gdict = gistools.reproject(infile, out_crs=dst_crs, geodata=geodata, outfile=outfile)
    ...
    def recrop(self, new_bbox, preview=False, force=False):
        ...
        if not lat_ok or not lon_ok:
            print(f"{Fore.RED}WARNING - Requested bbox for recropping is outside the bounds of the original image. ")
            print(f"Results may be unexpected.")

        # Calculate new height and width        
        width = int(abs(new_bbox[0] - new_bbox[2]) / geodata['res_x'])
        height = int(abs(new_bbox[1] - new_bbox[3]) / geodata['res_y'])

        print(f"{Fore.GREEN}Recropping images in Scene: {self.scene_folder}, Session: {self.session_name}...")
        time.sleep(0.5)
        for f in tqdm(self.img_filenames, colour='green'):
            if not preview:
                outfile = str(self.output_path / f)
            ...
            if self.type_is_geotiff:
                ...
                _, gdict = gistools.crop(infile, new_bbox, geodata=geodata, outfile=outfile)
    ...
    def adjust_alpha(self, opacity, preview=False):
        ...
        print(f"{Fore.GREEN}Adjusting transparency of images in Scene: {self.scene_folder}, Session: {self.session_name}...")
        time.sleep(0.5)
        for f in tqdm(self.img_filenames, colour='green'):
            if not preview:
                outfile = str(self.output_path / f)
    ...
    def add_timestamps(self, position='upper-left', tz=None, tzabbr=None, preview=False):
        """
        ...
        print(f"{Fore.GREEN}Adding timestamps to images in Scene: {self.scene_folder}, Session: {self.session_name}...")
        time.sleep(0.5)
        for i, f in enumerate(tqdm(self.img_filenames, colour='green')):
            ...
            tstr = datetime.datetime.isoformat(self.img_timestamps[i].astimezone(tz), timespec='seconds')
            tstr = tstr[0:-6]
            tstr = tstr.replace('T', ' ')
            tstr += ' ' + tzabbr
    ...
    def add_annotation(self, objtype, position, label=None, labelopts=None, timerange=None, **extraobjopts):
        newopts = {}
        if extraobjopts:
        ...
        if label:
            ...
            if labelopts:
                ...
                if not 'fontpath' in newopts[objtype]['label']:
                    if self.font['path']:
                ...
                if not 'fontsize' in newopts[objtype]['label']:
                    if self.font['size']:
        ...
        if timerange:
            ...
            if tstart >= self.img_timestamps[0] and tend <= self.img_timestamps[-1]:
                ...
                print(f"{Fore.RED}WARNING - Requested time range for annotation \'{objtype}\' is outside the range "
    ...
    def process_annotations(self, preview=False):
        ...
        print(f"{Fore.GREEN}Adding annotations to images in Scene: {self.scene_folder}, Session: {self.session_name}...")
        time.sleep(0.5)
        for i, f in enumerate(tqdm(self.img_filenames, colour='green')):
            if not preview:
                outfile = str(self.output_path / f)
            ...
            for opts in self.annotation_queue:
                ...
                if not skip:
                    ...
                    if self.type_is_geotiff:
                        ...
                        if counter == 0:
                            _tmpfile = tempfile.NamedTemporaryFile("w+b", suffix='.png', delete=False, delete_on_close=False)
    ...
    def add_overlay(self, oleditor:'GoesSceneEditor',
                    opacity=0.7,
                    dt_tol=20,
                    bbox_tol=2,
        ...
        if not preview:
            if not overlay_session_name:
                olfoldernum = []
                for _tup in os.walk(str(self.base_dir / "SceneEdits" / self.scene_folder)):
                ...
                overlay_session_name = self.session_name + '_' + 'Overlay_tmp_' + str(olnum)
                olfolder = self.base_dir / "SceneEdits" / self.scene_folder / overlay_session_name
            ...
            shutil.copyfile(str(self.output_path / "metadata.json"), str(olfolder / "metadata.json"))
            shutil.copyfile(str(self.output_path / "timestamps.csv"), str(olfolder / "timestamps.csv"))
        ...
        if skipped_idx_list:
            print(f"{Fore.RED}WARNING - Total of {len(skipped_idx_list)} image timestamps were outside the specified "
        ...
        # Create overlays
        print(f"{Fore.GREEN}Creating overlay images in Scene: {self.scene_folder}, Session: {overlay_session_name}...")
        time.sleep(0.5)
        counter = 0
        for idx in tqdm(matched_idx_list, colour='green'):            
            if self.type_is_geotiff:
                if not preview:
                    baseimg = gistools.geotiff_to_image(str(self.output_path / self.img_filenames[idx]))
                else:
                ...
                if not preview:
                    baseimg = Image.open(str(self.output_path / self.img_filenames[idx]))
                else:
            ...
            if not preview:
                olpath = gistools.adjust_alpha(str(oleditor.output_path / oleditor.img_filenames[idx]), opacity)
            else:
                if oleditor.type_is_geotiff:
                    olpath = gistools.adjust_alpha(oleditor.geodata['preview']['tif_cache'], opacity)
                else:
                    olpath = gistools.adjust_alpha(oleditor.geodata['preview']['png_cache'], opacity)
    ...
    def to_video(self, codec='mpeg4', fps=20):
        ...
        if codec == 'rawvideo' or codec == 'png':
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                print(f"{Fore.GREEN}Creating video for Scene: {self.scene_folder}, Session: {self.session_name}...")
    ...
    def _convert_geo_position(self, opts, geodata):
        ...
        if is_geographic and is_dms:
            if objtype == 'text':
                ...
                opts[objtype]['position'] = gistools.xy_from_latlon(gistools.convert_dms_to_dd(pos), geodata=geodata)
            elif objtype == 'circle':
                pos = opts[objtype]['centerpos']
                opts[objtype]['centerpos'] = gistools.xy_from_latlon(gistools.convert_dms_to_dd(pos), geodata=geodata)
            elif objtype == 'arrow':
                pos1 = opts[objtype]['start_position']
                pos2 = opts[objtype]['end_position']
                opts[objtype]['start_position'] = gistools.xy_from_latlon(gistools.convert_dms_to_dd(pos1), 
                                                                          geodata=geodata)
                opts[objtype]['end_position'] = gistools.xy_from_latlon(gistools.convert_dms_to_dd(pos2), 
                                                                        geodata=geodata)
            elif objtype == 'triangle':
                poslist = opts[objtype]['coords']
                newpos = [gistools.xy_from_latlon(gistools.convert_dms_to_dd(x) for x in poslist)]
                opts[objtype]['coords'] = newpos
        elif is_geographic and not is_dms:
            if objtype == 'text':
                pos = opts[objtype]['position']
                opts[objtype]['position'] = gistools.xy_from_latlon(pos, geodata=geodata)
            elif objtype == 'circle':
                pos = opts[objtype]['centerpos']
                opts[objtype]['centerpos'] = gistools.xy_from_latlon(pos, geodata=geodata)
            elif objtype == 'arrow':
                pos1 = opts[objtype]['start_position']
                pos2 = opts[objtype]['end_position']
                opts[objtype]['start_position'] = gistools.xy_from_latlon(pos1, geodata=geodata)
                opts[objtype]['end_position'] = gistools.xy_from_latlon(pos2, geodata=geodata)
            elif objtype == 'triangle':
                poslist = opts[objtype]['coords']
    ...
    def _create_tmp_files(self):
        tmpfile1 = tempfile.NamedTemporaryFile("w+b", suffix='.png', delete=False, delete_on_close=False)
